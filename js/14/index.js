...
addNewCardBtn.addEventListener('click', () => {
    if (!cardTitle.value.trim()) return ewMessage.warning("请填写卡片标题!");
    if (!cardContent.value.trim()) return ewMessage.warning("请填写卡片内容!");
    const card = {
...
deleteBtn.addEventListener('click', () => {
    if (!currentSelectCardID) return ewMessage.error("请选择需要删除的卡片!");
