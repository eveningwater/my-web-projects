...
class WhatToEat {
  ...
  initTime(status: boolean) {
    return new Promise((resolve, reject) => {
      ...
      if (date >= 7 && date <= 8) {
        ...
        if (status) {
          this.renderData.foodElementList = morningFoods.map((food) =>
        ...
        if (status) {
          this.renderData.foodElementList = noonFoods.map((food) =>
            this.createFood(food)
          );
        }
        ...
        if (status) {
          this.renderData.foodElementList = eveningFoods.map((food) =>
            this.createFood(food)
          );
        }
      } else {
        ...
        this.renderData.whatEat = "还没有到开饭时间，请等到开饭时间再来吧!";
      }
      if (currentTime) {
  ...
  trigger(target: any, key: string) {
    ...
    depsMap.get(key) &&
      depsMap.get(key).forEach((effect: Function) => effect && effect());
  }
  ...
  reactive(data: Object) {
    if (!this.isObject(data)) {
      return console.error(
        "[whatToEat warning]: the reactive data must be an object!"
      );
    }
    const that = this;
  ...
  render() {
    let foodElementHTML = this.renderData.foodElementList.reduce(
      (res: string, item: FoodElementType) => {
        res += `<span style="left:${item.left}px;top:${item.top}px;color:${item.color};position:absolute;" class="eat-random-food">${item.text}</span>`;
        return res;
      },
      ""
    );
